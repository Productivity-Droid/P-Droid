...
public class SmsListActivity extends ListActivity {
   ...
   // boolean n=true;
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == R.id.addsms) {
            startActivityForResult(new Intent(this, AddSmsActivity.class), 1);
        }else if (item.getItemId() == android.R.id.home) {
            onBackPressed();
        }else if (item.getItemId() == R.id.moresms) {
 ...
 */
    ...
    private SimpleCursorAdapter getSmsListAdapter() {
        SimpleCursorAdapter adapter = new SimpleCursorAdapter(
                ...
                new int[] {R.id.startdate, R.id.lastactive }
        );
        adapter.setViewBinder(new SimpleCursorAdapter.ViewBinder() {
            public boolean setViewValue(View view, Cursor cursor, int columnIndex) {

                TextView textView = (TextView) view;
                if (textView.getId() == R.id.lastactive) {
